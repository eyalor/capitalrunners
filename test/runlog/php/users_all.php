...
function getUsersRecords($conn) {
    ...
    $sql = "SELECT tl_runners.id as user_id, tl_runners.member_name as member_name,tl_runners.member_num as member_num, tl_runners.email as email, tl_runners.birthday as birthdate, tl_runners.m_show_profile as active_runner FROM tl_runners WHERE tl_runners.id = 18600094";
    $stmt = $conn->query($sql);
