...
"SELECT tl_events.id, tl_runners.member_name AS 'name',tl_runners.rpe_id AS 'rpe_id', tl_run_types.type, run_distance, run_time, warmup_distance, cooldown_distance, COALESCE(tl_events.notes, '') AS 'notes', tl_events.run_type_id, pulse, elevation, weight, date_format(tl_events.run_date,'%H:%i') as HourAndMinutes
    FROM tl_events
